...
### Тип вопроса
`c++_via_python_stepik` | `c_via_python_stepik`    (выбираем по ситуации)

### Настройка
ставим галочку рядом с "настроить"
<br>  

### Параметры шаблона
Тут происходит препроцессинг. Выполняются действия, которые необходимы ДО непосредственной проверки задания: получение сида и текста задания. Текст и сид попадают в текущую среду проверки, далее получить их можно с помощью `{{ task_text | e('py') }}` и `{{ task_seed | e('py') }}`. Реализуем препроцессинг по аналогии:
```python
...
### Настройка
Это обертка над проверкой через наш воркшоп. Тут прогоняются тесты и возвращается вердикт (**OK** или **Wrong answer**). Шаблон реализуем по аналогии:  
```python
# Клонирование репо и добавление воркшопа в пути (удалить после обновления кодранера)
from pathlib import Path
import subprocess
...
### Общее
**Название вопроса:** (вписываем осмысленное название)    
**Текст вопроса:** советую перекопировать текст задания с гита и отформатировать ручками, тк получение текста из препроцессинга ломает md форматирование  
...
По умилчанию выбран некорректный язык Ace с неработающей подсветкой (`C++`), меняем его на `c_cpp` 

<br>  

### Тестовые примеры
Создаем ОДИН тесткейс. Все тесткейсы воркшопа будут прогнаны в рамках одного тесткейса мудла. Проверка из настроек шаблона вернет **OK** или **Wrong answer**, тесткейс мудла это увидит и вынесет вердикт с выставлением баллов  
```
Тестовый пример 1: 1  
Стандартный ввод: 1  
