...
**1) Метод `fillNumberStack(int from, int to)`**
...
**3) Метод `isPalindrome(const std::string& str)`**
...
**4) Метод `isPalindromeIgnoringCaseAndSpaces(const std::string& str)`**
- Принимает: строку для проверки
...
**5) Метод `fillCharStack(const std::string& str)`**
