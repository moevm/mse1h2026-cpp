...
1. ООП:
Базовый класс Task   шаблонный по возвращаемому типу или использует CRTP (Curiously Recurring Template Pattern) для типобезопасности.
Но чтобы хранить разные задачи в одном контейнере, нужен общий базовый класс TaskBase.
execute() возвращает void, но заполняет внутренний Result<T>.
...
Контейнеры: std::deque<std::unique_ptr<TaskBase>> очередь задач; std::map<int, std::any> для хранения результатов.
Алгоритмы:
